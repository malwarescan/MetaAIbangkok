...
<div id="hs-application-sidebar" class="hs-overlay [--auto-close:lg] hs-overlay-open:translate-x-0 -translate-x-full duration-300 transform hidden hs-overlay-open:block fixed bottom-0 start-0 z-60 w-64 h-full bg-white border-gray-200 lg:block lg:translate-x-0 lg:end-auto lg:bottom-0" role="dialog" tabindex="-1" aria-label="Sidebar">
...
<div class="relative h-screen w-full lg:ps-64 pt-14 md:pt-16 overflow-hidden flex flex-col">
  <div class="flex-1 flex flex-col py-4 lg:py-6">
    ...
    <!-- Chat Messages -->
    <ul id="chat-messages" class="mx-auto space-y-5 max-w-4xl px-4 sm:px-6 lg:px-8 flex-1 overflow-y-auto"></ul>
  </div>
  ...
  <div class="max-w-4xl mx-auto p-3 sm:py-6">
    ...
    <div class="relative">
      <textarea id="chat-input" class="pb-12 block w-full bg-gray-100 border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Ask me anything..."></textarea>

      <!-- Toolbar on right -->
      <div class="fixed right-px p-2 rounded-b-lg bg-gray-100 w-max">
